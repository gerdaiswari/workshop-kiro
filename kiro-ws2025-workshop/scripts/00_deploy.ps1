...
# Verify prerequisites and select the Python 3 launcher available on this workstation.
...
$AccountId = (aws sts get-caller-identity --query Account --output text --region $Region --profile $Profile).Trim()
...
$bucketExists = $true
try { aws s3api head-bucket --bucket $Bucket --region $Region --profile $Profile 2>$null }
catch { $bucketExists = $false }
if (-not $bucketExists -or $LASTEXITCODE -ne 0) {
    Write-Host "Creating artifact bucket: $Bucket"
    if ($Region -eq 'us-east-1') {
        aws s3api create-bucket --bucket $Bucket --region $Region --profile $Profile | Out-Null
    } else {
...
# Clear stale logs from prior rollback
Write-Host 'Clearing stale bootstrap logs...'
aws s3 rm "s3://$Bucket/logs/$StackName/" --recursive --only-show-errors `
    --region $Region --profile $Profile 2>$null

# Cache XAMPP dependency
...
if ($remoteParts[0] -ne $XamppSize -or $remoteParts[1] -ne $XamppSha256) {
    ...
    aws s3 cp $XamppFile "s3://$Bucket/$XamppKey" `
        --metadata "sha256=$XamppSha256" --region $Region --profile $Profile --only-show-errors
} else {
    Write-Host "Reusing validated S3 dependency: s3://$Bucket/$XamppKey"
}

# Upload payload
aws s3 cp $ZipFile "s3://$Bucket/$ArtifactKey" --region $Region --profile $Profile --only-show-errors
...
if ($LASTEXITCODE -ne 0) {
    ...
    aws s3 ls "s3://$Bucket/logs/$StackName/" --recursive --region $Region --profile $Profile
    exit 1
}

# Save state
aws cloudformation describe-stacks --stack-name $StackName `
    --region $Region --profile $Profile --output json | Out-File "$DeployDir\stack.json" -Encoding utf8

$stateJson = @{
    stack_name = $StackName; region = $Region; profile = $Profile
...
$Dns = (aws cloudformation describe-stacks --stack-name $StackName `
    ...
    --output text --region $Region --profile $Profile).Trim()
